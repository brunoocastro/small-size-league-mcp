...
from config import DEFAULT_SITEMAP_URL, DEFAULT_URLS, URL_BLACKLIST
...
def process_urls(urls_info):
    ...
    all_urls = DEFAULT_URLS + extracted_urls

    website_urls = list(set(all_urls))
    
    filtered_urls = [url for url in website_urls if not any(keyword in url for keyword in URL_BLACKLIST)]
