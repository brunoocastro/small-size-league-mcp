...
def load_site(urls: List[str]) -> Tuple[List[Document], List[int]]:
    ...
    docs = []
    # Show the progress
    for url in tqdm(urls, desc="Processing URLs and loading page content", total=len(urls), unit="URLs"):
...
def save_full_website(documents: List[Document], full_website_file_path: str = FULL_WEBSITE_FILE_PATH):
